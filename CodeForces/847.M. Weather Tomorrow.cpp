...
int main () {
    ...
    int n, x, ratio, last_temp, ratio_changeCnt = 0;
    int equal = 0, increasing = 0, deccreasing = 0;
    ...
    {
        cin >> x;
        if(i != 2 && abs(last_temp-x) != ratio) ratio_changeCnt++;
        ratio = abs(last_temp - x);

        if(last_temp == x) equal = 1;
        if(last_temp < x) increasing = 1;
        if(last_temp > x) deccreasing = 1;
        last_temp = x;
    }
    if(ratio_changeCnt == 0 && (equal+increasing+deccreasing) == 1){
        if(equal == 1) cout << last_temp+ratio << endl;
        else if(increasing == 1) cout << last_temp+ratio << endl;
        else cout << last_temp-ratio << endl;
    }
    else cout << last_temp << endl;
