...
int main () {
    ...
    #endif // forthright48
    int phoneStorage, extraStorageNeeded, n, x, tCase;
    cin >> tCase;

    while(tCase--)
    {
        LL usedStorage = 0, freeSpace, uninstalledAppCnt = 0;
        cin >> phoneStorage >> extraStorageNeeded >> n;
        ...
        freeSpace = phoneStorage - usedStorage;
        while(freeSpace < extraStorageNeeded){
            freeSpace += pQ.top();
            pQ.pop();
            uninstalledAppCnt++;
        }

        cout << uninstalledAppCnt << endl;
    }
