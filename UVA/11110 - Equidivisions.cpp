...
int grid[102][102];
int vis[102][102];
int contiguousCell_cnt, row, col;
...
void FloodFill_DFS(int x, int y, int num)
{
    vis[x][y] = 1;
    contiguousCell_cnt++;
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        if(IsValid(u, v) == false || vis[u][v] != -1 || grid[u][v] != num) continue;
        FloodFill_DFS(u, v, num);
    }
}
...
*/
        ...
        for(int i = 0; i < N; i++)
        {
            for(int k = 0; k < N; k++)
            {
                ...
                contiguousCell_cnt = 0;
                FloodFill_DFS(i, k, grid[i][k]);

                if(contiguousCell_cnt != N){
                    res = false; break;
                }
            }
            if(res == false) break;
