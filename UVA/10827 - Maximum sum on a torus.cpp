...
int QsumCol[SZ+2][SZ+2];
int QsumRow[SZ+2][SZ+2];
int ROW, COL;
int maxSum = INT_MIN;
...
void findMaxSum()
{
    ...
    for (int topRow = 0; topRow < ROW; ++topRow)
	{
		for (int bottomRow = topRow; bottomRow < ROW; ++bottomRow)
        {
			...
			for (int i = 0; i < COL; ++i)
            {
				if(topRow == 0) sum = QsumCol[bottomRow][i];
				else sum = (QsumCol[bottomRow][i] - QsumCol[topRow-1][i]);
    ...
    for (int leftCol = 0; leftCol < COL; ++leftCol)
	{
		for (int rightCol = leftCol; rightCol < COL; ++rightCol)
        {
			...
			for (int i = 0; i < ROW; ++i)
            {
				if(leftCol == 0) sum = QsumRow[i][rightCol];
				else sum = (QsumRow[i][rightCol] - QsumRow[i][leftCol-1]);
...
int main()
{
    ...
    while(tCase--)
    {
		...
		memset(QsumCol, 0, sizeof(QsumCol));
        memset(QsumRow, 0, sizeof(QsumRow));
        ...
        for(int i = 0; i < ROW; i++){
            for(int k = 0; k < COL; k++)
            {
                ...
                if(k == 0) QsumRow[i][k] = Matrix[i][k];
                else QsumRow[i][k] = (QsumRow[i][k-1]+Matrix[i][k]);

				if(i == 0) QsumCol[i][k] = Matrix[i][k];
				else QsumCol[i][k] = (QsumCol[i-1][k]+Matrix[i][k]);
			}
        }

        findMaxSum();

        if(positiveNum) printf("%d\n",maxSum);
        else printf("%d\n",Max);
    }
