...
//tag -> DisjointSet, Disjoint_Set, Disjoint

int parent[SIZE+5], Rank[SIZE+5];
int x, y, Succesfull, Unsuccesfull;

void init(int n)
{
    Succesfull = 0, Unsuccesfull = 0;
    for(int i = 0; i <= n; i++){
        parent[i] = i; Rank[i] = 0;
    }
}
...
int main()
{
    ...
    for(int t = 1; t <= tCase; t++)
    {
        ...
        while(getline(cin, line) && line.size() > 0)
        {
            ...
            if(ch == 'c') Union(x, y); //cerr << "ok ===== " << endl;
            else
            {
                if(isUnion(x, y)) Succesfull++;
                else Unsuccesfull++;
            }
        }
        if(t != 1) printf("\n");
