...
vector < string > grid;
int row, col, currDir, stickerCnt = 0, toDo;
...
int rec(int x, int y, int i, int currDir)
{
    if(i == toDo) return stickerCnt;

    if(nextMove[i] == 'D') return rec(x, y, i+1, (currDir+1)%4);
    else if(nextMove[i] == 'E') return rec(x, y, i+1, (currDir+3)%4);
    else
    {
        int u, v;
        // 0 based index
        if(currDir == 0) u = x - 1, v = y;
        else if(currDir == 1) u = x, v = y + 1;
        else if(currDir == 2) u = x + 1, v = y;
        else if(currDir == 3) u = x, v = y - 1;

        if(IsValid(u, v) == false || grid[u][v] == '#') return rec(x, y, i+1, currDir);
        else
        {
            if(grid[u][v] == '*') stickerCnt++;
            grid[u][v] = '.';
            return rec(u, v, i+1, currDir);
        }
    }
}
...
int main()
{
    ...
    while(scanf("%d %d %d",&row,&col,&toDo) == 3)
    {
        if(!row && !col && !toDo) break;
