...
bool cmp(stateData a, stateData b){
    return(a.num_of_State>b.num_of_State || (a.num_of_State==b.num_of_State && a.language<b.language));
}
...
void DFS(int x, int y, char ch)
{
    ...
    for(int i = 0; i < 4; i++)
    {
        int u = x + fx4[i];
        int v = y + fy4[i];
        if(IsValid(u, v) == false || grid[u][v] != ch) continue;
        DFS(u, v, ch);
    }
}
...
int main()
{
    ...
    int ttCase;
    scanf("%d",&ttCase);
    for(int tCase = 1; tCase <= ttCase; ++tCase)
    {
        scanf("%d %d",&row,&col);
        grid.clear();
        string line;
        ...
        for(int i = 0; i < 26; i++)
        {
            Rank[i].language = i+97;
            Rank[i].num_of_State = 0;
        }
        ...
        for(int i = 0; i < row; i++)
        {
            for(int j = 0; j < col; j++)
            {
                ...
                char ch = grid[i][j];
                DFS(i, j, grid[i][j]);
                Rank[ch-'a'].num_of_State++;
            }
        }
        sort(Rank, Rank+26, cmp);

        printf("World #%d\n",tCase);
        for(int i = 0; i < 26; i++)
            if(Rank[i].num_of_State) printf("%c: %d\n",Rank[i].language, Rank[i].num_of_State);
    }
