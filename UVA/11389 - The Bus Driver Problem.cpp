...
int main()
{
    //read("input.txt");
    //write("output.txt");
    int n, d, r, routeLength;
    while(scanf("%d %d %d",&n,&d,&r) && (n||d||r))
    {
        while(!MaxValue.empty())    MaxValue.pop();
        while(!LowestValue.empty()) LowestValue.pop();


        for(int i = 0; i < n; i++){
            scanf("%d",&routeLength);
            MaxValue.push(routeLength);
        }

        for(int i = 0; i < n; i++){
            scanf("%d",&routeLength);
            LowestValue.push(routeLength);
        }

        long long res = 0;
        while(!MaxValue.empty())
        {
            int x = MaxValue.top();    MaxValue.pop();
            int y = LowestValue.top(); LowestValue.pop();
