...
int minDistance_to_Cover, mstEdgeCnt;
...
int MST()
{
    ...
    for(int i = 0; i < V.size(); i++)
    {
        ...
        if(x != y)
        {
            ...
            cost = cost + V[i].cost;
            minDistance_to_Cover = max(minDistance_to_Cover, V[i].cost);
            mstEdgeCnt++;
        }
    }
    return cost;
...
int main()
{
    ...
    int nCity, nRoad;
    while(scanf("%d %d",&nCity, &nRoad))
    {
        if(nCity == 0 && nRoad == 0) break;
        ...
        minDistance_to_Cover = 0;
        mstEdgeCnt = 0;
        int totRoadCost = MST();


        if(mstEdgeCnt == nCity-1) printf("%d\n",minDistance_to_Cover);
