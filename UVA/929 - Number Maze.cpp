...
void Dijkstra(int srcX, int srcY)
{
    ...
    //pii tmpPair = make_pair(srcX, srcY);

    pQ.push(node(make_pair(srcX, srcY), grid[srcX][srcY]));
    ...
    while(!pQ.empty())
    {
        ...
        for(int i = 0; i < 4; i++)
        {
            int u = x.dest.first  + fx4[i];
            int v = x.dest.second + fy4[i];

            if(IsValid(u, v) == false) continue;

            if(x.cost + grid[u][v] < Dist[u][v])
            {
                LL newCost = x.cost + grid[u][v];
                Dist[u][v] = newCost;
                pQ.push(node(make_pair(u,v), newCost));
            }
        }
    }
