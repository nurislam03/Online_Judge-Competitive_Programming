...
int main()
{
    ...
    while(scanf("%d %d",&nCity, &nRoad))
    {
        ...
        int city1, city2, lightingCost, totLightingCost = 0;
        for(int i = 0; i < nRoad; i++)
        {
            scanf("%d %d %d",&city1, &city2, &lightingCost);
            totLightingCost += lightingCost;

            C.st = city1; C.en = city2; C.cost = lightingCost;
            V.push_back(C);
        }
        sort(V.begin(), V.end());
        Initial(nCity);
        ...
        //mstEdgeCnt = 0;
        int mstLightingCost = MST();
        printf("%d\n",totLightingCost - mstLightingCost);
    }
