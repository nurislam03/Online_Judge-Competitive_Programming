...
int FloodFil_BFS(int x, int y)
{
    ...
    while(!q.empty())
    {
        ...
        int f = top.first;
        int s = top.second;
        if(f == endX && s == endY) return grid[f][s];
        ...
        for(int i = 0; i < 4; i++)
        {
            ...
            if(IsValid(u, v) == false || grid[u][v] != 0) continue;
            grid[u][v] = grid[f][s] + 1; // also use this to keep mark as visited cell.
...
int main()
{
    ...
    while(scanf("%d %d",&row,&col) == 2)
    {
        ...
        int nRow_containsBOMB;
        scanf("%d",&nRow_containsBOMB);

        while(nRow_containsBOMB--)
        {
            int rowNO, colNO, numOfBOMBS ;
            scanf("%d %d",&rowNO, &numOfBOMBS);
            while(numOfBOMBS--)
            {
                scanf("%d",&colNO);
                grid[rowNO][colNO] = -1;
        ...
        int ans = FloodFil_BFS(startX, startY);
        printf("%d\n",ans);
    }
