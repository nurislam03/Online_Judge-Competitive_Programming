...
vector < string > grid;
int row, col, regionCnt;

bool IsValid(int a, int b){
    return((a < row && a >= 0) && (b < col && b >= 0)) ? true:false;
}

void FloodFill_DFS(int x, int y, char ch)
{
    grid[x][y] = '1';
    regionCnt++;
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        //Horizontally Circular,(NOTE: only Horizontally Circular Not Vartically)
        if(v < 0) v = col-1;
        else if(v >= col) v = 0;

        if(IsValid(u, v) == false || grid[u][v] != ch) continue;
        FloodFill_DFS(u, v, ch);
    }
}
        ...
        for(int i = 0; i < row; ++i)
        {
            for(int j = 0; j < col; ++j)
            {
                if(grid[i][j] == ch){
                    ...
                    regionCnt = 0;
                    FloodFill_DFS(i, j, ch);
                    ans = max(ans, regionCnt);
                }
            }
        }
        printf("%d\n",ans);
