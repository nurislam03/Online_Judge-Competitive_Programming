...
void mark_Whole_Region(int x, int y)
{
    grid[x][y] = '.';
    for(int i = 0; i < 4; i++)
    {
        int u = x + fx4[i];
        int v = y + fy4[i];
        if(IsValid(u, v) == false || grid[u][v] != 'X') continue;
        mark_Whole_Region(u, v);
    }
}
...
void DFS(int x, int y)
{
    ...
    for(int i = 0; i < 4; i++)
    {
        int u = x + fx4[i];
        int v = y + fy4[i];
        if(IsValid(u, v) == false || grid[u][v] == '.') continue;
        DFS(u, v);
    }
}
