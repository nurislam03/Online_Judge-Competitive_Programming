...
bool cmp(holeName a, holeName b){
    return(a.holeArea>b.holeArea || (a.holeArea==b.holeArea && a.ch<b.ch));
}
    ...
    while(scanf("%d %d",&row,&col) == 2)
    {
        ...
        for(int i = 0; i < row; i++)
        {
            for(int j = 0; j < col; j++)
            {
                ...
                holeSize = 0;
                hole[holeCnt].ch = grid[i][j];
                DFS(i, j, grid[i][j]);
                hole[holeCnt++].holeArea = holeSize;
            }
        }

        sort(hole, hole+holeCnt, cmp);
        printf("Problem %d:\n",++tCase);
        for(int i = 0; i < holeCnt; i++)
            printf("%c %d\n",hole[i].ch, hole[i].holeArea);
    }
