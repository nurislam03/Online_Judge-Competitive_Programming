...
LL prime1 = 29, prime2 = 31;
LL Mod1 = 1000000000+7, Mod2 = 1000000000+9;
LL basePower[4][SIZE+10];


void powerCalculate(LL maxPower)
{
    basePower[0][0] = basePower[1][0] = basePower[2][0] = 1;
    for(int i = 1; i < maxPower; ++i){
        basePower[1][i] = (basePower[1][i-1] * prime1) % Mod1;
        basePower[2][i] = (basePower[2][i-1] * prime2) % Mod2;
    }
}


LL Hashing(string s, LL prime, LL modValue, int index)
{
    LL tot = 0;
    for(int i = 0; s[i]; i++){
        tot += (basePower[index][i]*s[i]) % modValue;
    }
    return (tot%modValue);
}


void check_Virus(string s)
{
    LL Hash1 = Hashing(s, prime1, Mod1, 1) % Mod1;
    LL Hash2 = Hashing(s, prime2, Mod2, 2) % Mod2;
    ...
    for(int i = 0; s[i]; ++i)
    {
        for(int j = 0; j < 26; ++j)
        {
            ...
            LL tmp1 = (Hash1 - (basePower[1][i] * s[i])%Mod1) % Mod1;
            LL tmp2 = (Hash2 - (basePower[2][i] * s[i])%Mod2) % Mod2;
            if(tmp1 < 0) tmp1 = tmp1 + Mod1;
            if(tmp2 < 0) tmp2 = tmp2 + Mod2;

            LL tmpHash1 = (tmp1 + (basePower[1][i]*ch)) % Mod1;
            LL tmpHash2 = (tmp2 + (basePower[2][i]*ch)) % Mod2;

            if(HashMap.find(make_pair(tmpHash1, tmpHash2)) != HashMap.end()){
                pairCnt += HashMap[make_pair(tmpHash1, tmpHash2)];
            }
        }
    }

    HashMap[make_pair(Hash1, Hash2)]++;
}
