...
int main()
{
    ...
    //write("output.txt");
    int n, k, ansFloor;
    double u, v, initDown = 0.0, bestTiming = 0.0;
    double liftUP = 0.0, totTime = 0.0, byStair = 0.0, totBack = 0.0;
    while(scanf("%d %d %lf %lf",&n, &k, &u, &v) != EOF)
    {
        initDown = (v*(k-1))+15.0;
        bestTiming = (u*(n-1));
        ansFloor = 1;
        ...
        for(int i = n; i > 1; i--)
        {
            ...
            totBack = max(initDown, byStair) + liftUP;
            totTime = (totBack)+((i-1)*v)+5.0;

            if((totTime+eps) < bestTiming){
                bestTiming = totTime;
                ansFloor = i;
            }
        }
