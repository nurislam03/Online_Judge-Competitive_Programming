...
int Find[SZ+10][SZ+10];
int vis[SZ+10];
pair<int, int> Pair[5000005];
int valid[SZ+5];
vector < int > build;
int nodeCnt, n, m;
...
int main()
{
    ...
    for(int i = 1; i <= event; i++){
        ...
        if(type == 1){
            valid[++nodeCnt] = i;//////
            if(v == 0) adj[u].push_back(nodeCnt);
            else adj[nodeCnt].push_back(u);
        }
        else{
            build.push_back(n+i);
            Pair[cnt++] = make_pair(u,v);
        }
    }
    preCalculate();
    ...
    for(int i = 0; i < cnt; i++){
        ...
        v = Pair[i].second;
        int tmp = build[i];
        if(valid[u] > tmp || valid[v] > tmp){
            printf("No\n");
            continue;
        }
        if(Find[u][v] == 1) printf("Yes\n");
        else printf("No\n");
    }
