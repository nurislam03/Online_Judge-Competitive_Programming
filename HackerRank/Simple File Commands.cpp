...
priority_queue<int , vector<int> , greater<int > > LowestValue[100000+100];
int maxLength[100000+100];
map <string,int> Map;


Pair nameConvert(string s)
{
    string ss = "";
    int num = 0;
    int len = s.size()-1;
    ...
    for(int i = s.size()-2,j = 0; s[i] != '('; i--,j++){
        int tmp = s[i]-'0';
        num += (tmp* pow(10,j));
    }
    return Pair(ss,num);
...
int createFile(string s, int id)
{
    int newId;
    if(LowestValue[id].empty()){
        maxLength[id] += 1;
        newId = maxLength[id];
        //cout << endl;
        //debug2(id);
        ...
        newId = LowestValue[id].top();
        LowestValue[id].pop();
    }
    return newId;
}

void deleteFile(string s, int id, int num){
    LowestValue[id].push(num);
}
...
int main()
{
    ...
    Pair val;
    string name;
    int ID, idCnt = 0, fileNum;

    Map.clear();
    memset(maxLength, -1, sizeof(maxLength));
    for(int i = 0; i <= 100000; i++){
        while(!LowestValue[i].empty())
            LowestValue[i].pop();
    }
    ...
    while(tCase--)
    {
        ...
        if(cmnd == "crt"){
            ...
            if(Map[f1]) ID = Map[f1];
                ...
                Map[f1] = ++idCnt;
                ID = Map[f1];
                //cout << "id count Inner = " << idCnt << endl;
            }
            //cout << "id count = " << ID << endl;//
            int newId = createFile(f1, ID);

            if(newId == 0) cout << "+ " << f1 << endl;
            else cout << "+ " << f1 << "(" << newId << ")" << endl;
        }
        else if(cmnd == "del"){
            val = nameConvert(f1);
            ...
            deleteFile(name, ID, fileNum);
            //debug1(name);
            if(fileNum == 0) cout << "- " << name << endl;
            else cout << "- " << name << "(" << fileNum << ")" << endl;
            ...
            deleteFile(name, ID, fileNum);
            if(fileNum == 0) cout << "r " << name;
            else cout << "r " << name << "(" << fileNum << ")";
            ...
            int newVal = createFile(f2, ID);
            if(newVal == 0) cout << " -> " << f2 << endl;
            else cout << " -> " << f2 << "(" << newVal << ")" << endl;
        }
    }
