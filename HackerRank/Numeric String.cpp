...
LL summation()
{
    ...
    for(int i = 0, p = k-1; i < k; i++,--p){
        int tmp = (s[i]-'0');
        tmpSum += (BigMod(b,p,m)*tmp)%m;
    }

    if(tmpSum%m < 0) tmpSum = ((tmpSum%m)+m);
    else tmpSum = (tmpSum%m);

    sum += (tmpSum%m);

    for(int i = 0,kk = k; kk < s.size(); i++,kk++){
        int tmpi = (s[i]-'0');
        int tmpk = (s[kk]-'0');

        tmpSum = (tmpSum - (tmpi*BigMod(b,k-1,m)));
        tmpSum = ((tmpSum*(b%m))+(tmpk*BigMod(b,0,m)));
...
void making_subset()
{
    ...
    for(int i = 0; i < len; i++){
        ...
        //debug1(newS);
        if(Map[newS]) sum += Map[newS];
        else sum += base_conversion(newS);
    }
}
*/
