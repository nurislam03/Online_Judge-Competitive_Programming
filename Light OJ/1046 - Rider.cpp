    ...
    while(!q.empty())
    {
        ...
        for(int i = 0; i < 8; i++)
        {
            ...
            if(IsValid(u, v) && !vis[u][v])
            {
                vis[u][v] = true;
                ++reachedRiderCnt[u][v];
                totMove[u][v] += jump/k + (jump%k != 0);

                q.push(node(u, v, jump));
            }
        }
    }
}
...
int main()
{
    ...
    {
        scanf("%d %d",&row, &col);
        memset(reachedRiderCnt, 0, sizeof reachedRiderCnt);
        memset(totMove, 0, sizeof totMove);
        ...
        for(int i = 0; i < row; i++)
        {
            for(int j = 0; j < col; j++)
            {
                if(totRiderCnt == reachedRiderCnt[i][j])
                    ans = min(ans, totMove[i][j]);
            }
        }
        if(ans == INF) ans = -1;
        printf("Case %d: %d\n",t, ans);
