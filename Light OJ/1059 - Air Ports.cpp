...
map <string, int> Map;
int mstEdgeCnt, Parent[SIZE+5], Rank[SIZE+5];
...
int main()
{
    ...
    for(int tCase = 1; tCase <= nCase; tCase++)
    {
        ...
        int nCity, nRoad, airport_BuildingCost;
        scanf("%d %d %d",&nCity, &nRoad, &airport_BuildingCost);

        int city1, city2, roadCost;
        for(int i = 0; i < nRoad; i++)
        {
            scanf("%d %d %d",&city1, &city2, &roadCost);
            if(roadCost >= airport_BuildingCost) continue;

            C.st = city1; C.en = city2; C.cost = roadCost;
            V.push_back(C);
        }
        sort(V.begin(), V.end());
        Initial(nCity);
        ...
        int totalAirportCnt = (nCity - mstEdgeCnt);
        int totalCost = totRoadCost + (totalAirportCnt * airport_BuildingCost);


        printf("Case %d: %d %d\n",tCase, totalCost, totalAirportCnt);
    }
