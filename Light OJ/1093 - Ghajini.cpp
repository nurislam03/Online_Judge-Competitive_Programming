...
{
    if(a == b){
        tree[node].maxVal = ara[a];
        tree[node].minVal = ara[a];
        return;
    }
    ...
    tree[node].maxVal = max(tree[left].maxVal, tree[right].maxVal);
    tree[node].minVal = min(tree[left].minVal, tree[right].minVal);
}
...
TREE query(int node, int a, int b, int i, int j)
{
    ...
    return TREE(max(q1.maxVal, q2.maxVal), min(q1.minVal, q2.minVal));
}
...
int main()
{
    ...
    for(int T = 1; T <= tCase; T++)
    {
        ...
        for(int i = 1; i <= n; i++)
        {
            TREE tmpDiff = query(1, 1, n, i, (i+d-1));
            int tmpMax = tmpDiff.maxVal;
            int tmpMin = tmpDiff.minVal;

            MaxDiff = max(MaxDiff, abs(tmpMax - tmpMin));
        }
        printf("Case %d: %d\n",T, MaxDiff);
    }
